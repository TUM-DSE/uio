...
InterceptionResult UShellCmdInterceptor::intercept(const std::string &in)
{
	...
	if (!tokens.empty()) {
		...
		if (command == "cwd") {
			...
			// TODO add options: assert termination, assert dived
			// not by zero, etc.
			EBPFVerifier verifier(ebpf_verifier_options);

			// verify bpf file
			bool allOK = true;
			try {
				auto bpfFileSections =
				    verifier.getSections(bpfFileHost);
				...
				for (const auto &section : bpfFileSections) {
					...
					std::cout << "Verifying section: "
						  << section;
					auto verifyResult = verifier.verify(
					    bpfFileHost, section);
					std::cout
